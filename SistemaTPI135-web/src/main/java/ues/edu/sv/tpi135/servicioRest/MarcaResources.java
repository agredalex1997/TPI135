...
public class MarcaResources implements Serializable{
    ...
    @Path("/edit/{idMarca}/{nombre}/{activo}")
    @PUT
    @Consumes(MediaType.APPLICATION_JSON)
    public Response edit(@PathParam("idMarca") int idMarca, @PathParam("nombre") String nombre, @PathParam("activo") boolean activo){       
        Response respuesta = Response.status(Response.Status.NOT_FOUND).build();
        if(ejbMarca != null){
            Marca editMarca = new Marca(idMarca, nombre, activo);
            ejbMarca.edit(editMarca);
            respuesta = Response.status(Response.Status.OK).build();
        }
        return respuesta;
    }
