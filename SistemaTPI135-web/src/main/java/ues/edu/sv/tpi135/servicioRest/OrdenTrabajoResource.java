...
public class OrdenTrabajoResource implements Serializable{
    ...
    @Path("/edit/{idOrdenTrabajo}/{fecha}")
    @PUT
    @Consumes(MediaType.APPLICATION_JSON)
    public Response edit(@PathParam("idOrdenTrabajo") Integer idOrdenTrabajo, @PathParam("fecha") Date fecha){       
        Response respuesta = Response.status(Response.Status.NOT_FOUND).build();
        if(ejbOrdenTrabajo != null){
            OrdenTrabajo editOrdenTrabajo = new OrdenTrabajo(idOrdenTrabajo, fecha);
            ejbOrdenTrabajo.edit(editOrdenTrabajo);
            respuesta = Response.status(Response.Status.OK).build();
        }
