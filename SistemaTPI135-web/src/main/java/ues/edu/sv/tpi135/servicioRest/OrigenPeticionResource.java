...
public class OrigenPeticionResource implements Serializable{
    ...
    @Path("/edit/{idOrigen}/{nombre}")
    @PUT
    @Consumes(MediaType.APPLICATION_JSON)
    public Response edit(@PathParam("idOrigen") Integer idOrigen, @PathParam("nombre") String nombre){
        Response respuesta = Response.status(Response.Status.NOT_FOUND).build();
        if(ejbOrigenPeticion != null){
            OrigenPeticion editOrigenPeticion = new OrigenPeticion(idOrigen, nombre);
            ejbOrigenPeticion.edit(editOrigenPeticion);
            respuesta = Response.status(Response.Status.OK).build();
        }
        return respuesta;
    }
