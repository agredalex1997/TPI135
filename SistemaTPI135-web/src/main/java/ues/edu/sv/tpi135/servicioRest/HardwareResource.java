...
public class HardwareResource implements Serializable{
    ...
    @Path("/edit/{idHardware}/{nombre}")
    @PUT
    @Consumes(MediaType.APPLICATION_JSON)
    public Response edit(@PathParam("idHardware") int idHardware, @PathParam("nombre") String nombre){       
        Response respuesta = Response.status(Response.Status.NOT_FOUND).build();
        if(ejbHardware != null){
            Hardware editHardware = new Hardware(idHardware, nombre);
            ejbHardware.edit(editHardware);
            respuesta = Response.status(Response.Status.OK).build();
        }
        return respuesta;
    }
