...
public class PeticionResource implements Serializable{
    ...
    @Path("/edit/{idPeticion}/{fecha}")
    @PUT
    @Consumes(MediaType.APPLICATION_JSON)
    public Response edit(@PathParam("idPeticion") Integer idPeticion, @PathParam("fecha") Date fecha){       
        Response respuesta = Response.status(Response.Status.NOT_FOUND).build();
        if(ejbPeticion != null){
            Peticion editPeticion = new Peticion(idPeticion, fecha);
            ejbPeticion.edit(editPeticion);
            respuesta = Response.status(Response.Status.OK).build();
        }
